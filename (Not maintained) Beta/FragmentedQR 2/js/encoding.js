...
var Encoding = {
  ...
  /**
   * å…¨è§’è‹±æ•°è¨˜å·æ–‡å­—ã‚’åŠè§’è‹±æ•°è¨˜å·æ–‡å­—ã«å¤‰æ›
   *
   * Convert the ascii symbols and alphanumeric characters to
   *   the zenkaku symbols and alphanumeric characters
   *
   * @example
   *   console.log(Encoding.toHankakuCase('ï¼¨ï½…ï½Œï½Œï½ ï¼·ï½ï½’ï½Œï½„ï¼ ï¼‘ï¼’ï¼“ï¼”ï¼•'));
   *   // 'Hello World! 12345'
   *
  ...
  /**
   * åŠè§’è‹±æ•°è¨˜å·æ–‡å­—ã‚’å…¨è§’è‹±æ•°è¨˜å·æ–‡å­—ã«å¤‰æ›
   *
   ...
   *   // 'ï¼¨ï½…ï½Œï½Œï½ ï¼·ï½ï½’ï½Œï½„ï¼ ï¼‘ï¼’ï¼“ï¼”ï¼•'
   *
   * @param {Array.<number>|TypedArray|string} data The input unicode data
   * @return {Array.<number>|string} The conveted data
   */
  ...
  /**
   * å…¨è§’ã‚«ã‚¿ã‚«ãƒŠã‚’å…¨è§’ã²ã‚‰ãŒãªã«å¤‰æ›
   *
   * Convert to the zenkaku hiragana from the zenkaku katakana
   *
   * @example
   *   console.log(Encoding.toHiraganaCase('ãƒœãƒãƒ´ã‚¡ã‚¢ã‚£ã‚¤ã‚¥ã‚¦ã‚§ã‚¨ã‚©ã‚ª'));
   *   // 'ã¼ã½ã†ã‚›ãã‚ãƒã„ã…ã†ã‡ãˆã‰ãŠ'
   *
   * @param {Array.<number>|TypedArray|string} data The input unicode data
   * @return {Array.<number>|string} The conveted data
   */
  ...
  /**
   * å…¨è§’ã²ã‚‰ãŒãªã‚’å…¨è§’ã‚«ã‚¿ã‚«ãƒŠã«å¤‰æ›
   *
   * Convert to the zenkaku katakana from the zenkaku hiragana
   *
   * @example
   *   console.log(Encoding.toKatakanaCase('ã¼ã½ã†ã‚›ãã‚ãƒã„ã…ã†ã‡ãˆã‰ãŠ'));
   *   // 'ãƒœãƒãƒ´ã‚¡ã‚¢ã‚£ã‚¤ã‚¥ã‚¦ã‚§ã‚¨ã‚©ã‚ª'
   *
   * @param {Array.<number>|TypedArray|string} data The input unicode data
   * @return {Array.<number>|string} The conveted data
   */
  toKatakanaCase: function(data) {
    ...
    while (i < len) {
      ...
      if (c >= 0x3041 && c <= 0x3096) {
        if ((c === 0x308F || // ã€Œã‚ã€ + ã€Œã‚›ã€ => ã€Œãƒ¯ã‚›ã€
             c === 0x3092) && // ã€Œã‚’ã€ + ã€Œã‚›ã€ => ã€Œãƒ²ã‚›ã€
            i < len && data[i] === 0x309B) {
  ...
  /**
   * å…¨è§’ã‚«ã‚¿ã‚«ãƒŠã‚’åŠè§’ï½¶ï¾€ï½¶ï¾…ã«å¤‰æ›
   *
   * Convert to the hankaku katakana from the zenkaku katakana
   *
   * @example
   *   console.log(Encoding.toHankanaCase('ãƒœãƒãƒ´ã‚¡ã‚¢ã‚£ã‚¤ã‚¥ã‚¦ã‚§ã‚¨ã‚©ã‚ª'));
   *   // 'ï¾Žï¾žï¾Žï¾Ÿï½³ï¾žï½§ï½±ï½¨ï½²ï½©ï½³ï½ªï½´ï½«ï½µ'
   *
   * @param {Array.<number>|TypedArray|string} data The input unicode data
   * @return {Array.<number>|string} The conveted data
   */
  toHankanaCase: function(data) {
    ...
    while (i < len) {
      ...
      // ã€Œãƒ´ã€, ã€Œãƒ¯ã€+ã€Œã‚›ã€, ã€Œãƒ²ã€+ã€Œã‚›ã€
  ...
  /**
   * åŠè§’ï½¶ï¾€ï½¶ï¾…ã‚’å…¨è§’ã‚«ã‚¿ã‚«ãƒŠã«å¤‰æ› (æ¿éŸ³å«ã‚€)
   *
   * Convert to the zenkaku katakana from the hankaku katakana
   *
   * @example
   *   console.log(Encoding.toZenkanaCase('ï¾Žï¾žï¾Žï¾Ÿï½³ï¾žï½§ï½±ï½¨ï½²ï½©ï½³ï½ªï½´ï½«ï½µ'));
   *   // 'ãƒœãƒãƒ´ã‚¡ã‚¢ã‚£ã‚¤ã‚¥ã‚¦ã‚§ã‚¨ã‚©ã‚ª'
   *
   * @param {Array.<number>|TypedArray|string} data The input unicode data
   * @return {Array.<number>|string} The conveted data
      ...
      if (c > 0xFF60 && c < 0xFFA0) {
        ...
        if (i + 1 < len) {
          ...
          // ã€Œï¾žã€ + ã€Œï½¦ã‚›ã€
            ...
            code = 0x30FA;
            i++;
            // ã€Œï¾žã€ + ã€Œã‚«ã€ - ã€Œã‚³ã€ or ã€Œãƒã€ - ã€Œãƒ›ã€
          } else if (next === 0xFF9E &&
                     ((c > 0xFF75 && c < 0xFF85) ||
                      (c > 0xFF89 && c < 0xFF8F))) {
            code++;
            i++;
            // ã€Œï¾Ÿã€ + ã€Œãƒã€ - ã€Œãƒ›ã€
          } else if (next === 0xFF9F &&
                     (c > 0xFF89 && c < 0xFF8F)) {
            code += 2;
            i++;
  ...
  /**
   * å…¨è§’ã‚¹ãƒšãƒ¼ã‚¹ã‚’åŠè§’ã‚¹ãƒšãƒ¼ã‚¹ã«å¤‰æ›
   *
  ...
  /**
   * åŠè§’ã‚¹ãƒšãƒ¼ã‚¹ã‚’å…¨è§’ã‚¹ãƒšãƒ¼ã‚¹ã«å¤‰æ›
   *
